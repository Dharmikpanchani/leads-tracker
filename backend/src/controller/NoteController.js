...
export const deleteNote = async (req, res) => {
  try {
    ...
    if (!note) {
      return ResponseHandler(res, StatusCodes.NOT_FOUND, responseMessage.NOTE_NOT_FOUND);
    }

    await Note.findByIdAndDelete(noteId);

    return ResponseHandler(res, StatusCodes.OK, responseMessage.NOTE_DELETED, { id: noteId });
  } catch (error) {
    return CatchErrorHandler(res, error);
  }
};
